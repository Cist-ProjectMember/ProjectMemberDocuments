...
実際戻り値がIntegerである場合は`null`である可能性があるのでtry-catchを用いてNullPointerExceptionのチェックを行う必要がある。見た目で戻り値の型が判断できない場合はできる限り`var`の利用は控えて、しっかりと型の指定をするとよい。このように思わぬ点でエラーが発生してしまう可能性があるために、大規模な開発では話し合っておくべき必要があると思う。(個人的意見)
...
Enumとは、列挙型と呼ばれているクラスである。

### 使い方

#### 利用方法

Enumクラスと呼ばれているだけあって、一応クラスの一部であるため、いつものクラスの記述の仕方と命名規則などは同じである。

#### 利用例

実際にどのような場面で利用することができるのかを例を用いて説明する。
...
一番簡単なEnumの記述方法はこのようになる。さらにEnumは定数として利用する場面が多いので列挙子はすべて大文字で記述する。
...
実際、匿名クラスなんてどんなときに使うの(?)と思うかもしれない。実際使う場面は限られている。思いつく簡単な場合を考えてみる。  
...
はっきり言って僕自身も完璧には理解していないし、こんな時使うぜ。くらいにしか思っていない。  
...
さらに明らかな違いは、既存の方法だとひとつひとつ変換するたびにListに追加しているのに対し、streamを利用した場合では｢中間操作｣を終えて形を変換し終わった後に、終端操作を行いListに変換していることである。  
...
```
 ...
 このようになる。今回は既存のfor文でのやり方は用意していないが、どう考えてもfor文より見やすいと思わないだろうか?僕は見やすいと思う。  
 ...
 今更であるが、`.forEach(s -> System.out.println(s))`の`s -> System.out.println(s)`ってなんやねん って思うかもしれないが、これはLambda式という記法である。`->`は __アロー演算子__ と言う。これらに関しての説明は今回はしないが`Java Lambda式`とでも調べたら出てくると思う。ちなみに`System.out::println`はメソッド参照と言う。
