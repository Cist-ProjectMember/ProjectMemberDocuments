...
次回はJavaのまとめを行います。  
DB編に入る前にもう一度Javaの基礎を確認するための時間ですので、この時間で基本文法を毎回調べる必要がないくらいまでは知識をつけておきましょう。
DBでは取得した値をListに詰め込んだり、アクセス時に例外が発生したりと、これまでの内容が当たり前のように出てきます。  
また、DBを使用した場合の例外処理では、
`try-with-resource`
という通常のtry-catchとは若干異なる例外処理を行うため、事前に調べておくと良いかもしれません。  
