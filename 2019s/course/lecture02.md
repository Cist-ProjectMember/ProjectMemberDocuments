...
クラス名とクラスが定義されているファイル(xxxx.java)の名前は統一すること
...
class Television {
	int channel;
	Television(int channel){ this.channel = channel; }
}
```
...
class Television {
	Plug powerSupply;		// 電源
	void jointPlug(Plug plug){ this.powerSupply = plug; }
}
```
...
Carをmain()メソッドでインスタンス化しrun()を呼び出した後、ドット演算子を用いてfuel=20に設定してもう一度run()を呼び出せ
