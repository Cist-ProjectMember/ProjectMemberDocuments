...
また、変数名を指定している課題であっても、匿名クラスが使用できる場合に限って匿名クラスの使用を許可する。
...
また、`PasswordTextField`型の`userPasswordTextField` にPasswordTextFieldをインスタンス化する(コンストラクタに上で作成した`userPasswordModel`を渡す)。
